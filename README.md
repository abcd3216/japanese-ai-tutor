...
還有一個我自己覺得很好玩的「**單字小恐龍**」小遊戲。你的學習紀錄會自動存到雲端，
之後還能每天寄一封學習日誌到你的 Gmail（這個是我加好玩的，選配功能）。
...
- **🦖 玩單字小恐龍**
  ...
  你要判斷這個日文是不是題目的意思：對就按 `1`、錯就按 `2`。限時 60 秒，玩完成績會存起來。
...
1. **只想玩玩看 → 有網址最快**
   如果我（或其他人）已經把它架在雲端，你只要打開網址就能直接用，**什麼都不用裝**。

2. **想自己架一份、或研究我怎麼寫的 → 照下面步驟**
   在自己電腦上跑，或部署成你自己的雲端網址（部署方式我寫在 [`DEPLOY.md`](DEPLOY.md)）。

---

## 想自己在電腦上跑跑看？

> 這段是給想自己動手跑的人看的。需要先有 Python（3.10 以上）。
> 我盡量把每一步在做什麼都寫清楚了，照著做應該就能跑起來。
...
2. `database/seed_vocabulary.sql` — 匯入 160 個單字（N5、N4 各 80 個，是我一個一個整理的 😅）
...
- **一開始連很慢、卡住？**
  ...
  等一下就好，**這是正常的，不是壞掉**（我特別做了自動重試，讓它不會直接噴錯誤）。
...
這個要另外有一台 n8n 才能用，算是加分的小功能，不影響網站本身。
有興趣的話看 [`n8n/README.md`](n8n/README.md)。

---

這是我的個人學習專案，一定還有很多可以做得更好的地方，
如果你有任何想法或發現 bug，都很歡迎跟我說 🙏
