...
你的學習紀錄會自動存到雲端資料庫，之後還能每天寄一封學習日誌到你的 Gmail。
...
1. **只想用 → 拿網址就好**
   如果架設者已經把它部署到雲端，你只要打開他給的網址（例如
   `https://japanese-ai-tutor.streamlit.app`）就能直接用，**什麼都不用裝**。

2. **想自己架一份 → 照下面的步驟**
   在自己電腦上跑，或部署成自己的雲端網址（部署方式見 [`DEPLOY.md`](DEPLOY.md)）。
